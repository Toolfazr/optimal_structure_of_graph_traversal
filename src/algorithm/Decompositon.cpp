...
BfsResult bfsFarthest(const Graph& g, Index start, const std::vector<char>& allowed) {
    ...
    while (!q.empty()) {
        ...
        for (Index v : neighbors) {
            ...
            result.parent[v] = u;
            q.push(v);
            if (result.dist[v] > result.dist[result.farthest]
                || (result.dist[v] == result.dist[result.farthest] && v < result.farthest)) {
                result.farthest = v;
            }
        }
    }
...
std::vector<std::vector<Index>> Decomposition::getRanks(const Graph& g) {
    ...
    return decompose(g, nodes, allowed);
}

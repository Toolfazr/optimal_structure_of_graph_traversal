...
namespace {
    ...
    static std::vector<Index> pickHubsMeanStd(const Graph& g) {
        ...
        for (int v = 0; v < n; ++v) {
            double d = static_cast<double>(g.getNeighbors(v).size());
            if (d >= th) hubs.push_back(static_cast<Index>(v));
        }
        return hubs;
    }
...
double Metrics::highDegreeSpacingImpl(const Graph& graph, const TraversalTrace& t) {
    ...
    if (n <= 0 || t.order.size() < 2) return 0.0;
    ...
    for (int i = 0; i < K; ++i) {
        ...
        if (pv >= 0) p.push_back(pv); // 非连通时可能 -1，跳过
    }

    if (p.size() < 2) return -1.0; // 无法计算间距
    ...
    if (deltaStar <= 0.0) return 1.0; // 退化情况，视为“最好”
...
double Metrics::branchSuspensionImpl(const TraversalTrace& t) {
    ...
    for (int v = 0; v < n; ++v) {
        ...
        if (p >= 0 && p < n) {
            int pv = pos[static_cast<std::size_t>(v)];
            if (pv >= 0) childPos[static_cast<std::size_t>(p)].push_back(pv);
        }
    }
